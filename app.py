...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        sample_students = [
            ("S001", "John ", "II - ECE", 1),
            ("S002", "Smith", "I - ECE", 2),
            ("S003", "Robert", "IV - ECE", 1),
            ("S004", "Johnson", "III - ECE", 3),
        ...
        try:
            ...
            if self.ai_model and face_roi.size > 0:
                try:
                    ...
                    if (
                        "gender" in ai_prediction
                        and ai_prediction["gender"] != "unknown"
                    ):
                        results["gender"] = ai_prediction["gender"]
                        gender = ai_prediction["gender"]
                except Exception as e:
            ...
            def detect_footwear(image, face_region, ai_prediction):
                ...
                is_full_body_visible = bottom_of_body <= image_height * 1.1

                if not is_full_body_visible:
                    return "Not Visible"

                # Check 2: AI detection first (most accurate)
                ...
                foot_y = min(y + int(h * 1.5), image_height - 50)
                foot_h = min(150, image_height - foot_y)

                if foot_y >= image_height - 20 or foot_h < 30:
                    return "Unknown"

                foot_region = image[
                    foot_y : foot_y + foot_h,
                    max(0, x - w // 2) : min(image_width, x + w * 2),
                ]
...
def process_image(image_path, is_upload=False):
    try:
        ...
        for x, y, w, h in faces:
            ...
            # Detect violations with gender awareness
            violations = []
            ...
            # 2. ID CARD - Check only if visible
            ...
            result = {
                ...
                "analysis": {
                    ...
                    "hair_neat": bool(
                        not analysis["beard"] if gender == "Male" else True
                    ),
                ...
                "combined_result": {
                    ...
                    "confidence": (
                        ai_result.get("compliance_score", 0.5) if ai_result else 0.5
                    ),
                ...
                "grooming_result": {
                    ...
                    "compliance_score": (
                        ai_result.get("compliance_score", 0.8) if ai_result else 0.8
                    ),
